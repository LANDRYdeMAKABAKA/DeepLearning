...
class RNN_model(nn.Module):
    ...
    def forward(self,sentence,is_test = False):
        ...
        # ???
        # 将输入传递给LSTM层
        h0 = torch.zeros(2, batch_input.size(0), self.lstm_dim).to(batch_input.device)  # 初始化隐藏状态
        c0 = torch.zeros(2, batch_input.size(0), self.lstm_dim).to(batch_input.device)  # 初始化细胞状态
